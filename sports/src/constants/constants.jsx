import temple from "../assets/temples.png";
import participants from "../assets/participants.png";
import torch from "../assets/events.png";
import winner from "../assets/winners.png";
// import supporters from "../assets/supporters.png";


//rules in English
export const rules = [
  "Only members from the 16 temples (Padmashali) are allowed to participate in all competitions.",
  "A maximum of 3 participants per event from each temple are allowed in individual categories. Any one participant may compete in up to a maximum of 3 events. (This rule does not apply to those below 10 years and above 60 years of age.)",
  "If a minimum of 5 participants are not registered in any event, that event will be canceled.",
  "As per the Mahasabha rules, each temple must register by paying ₹1000.",
  "The sports meet will begin at 9:15 AM with the 800m race across all categories, followed by volleyball and throwball matches.",
  "Participants for volleyball, throwball, and tug-of-war must register with a copy of their Aadhaar card and a letter from their respective temple, through the sports coordinator or sports secretary by 1-12-2025.",
  "The volleyball and throwball matches will commence at 9:15 AM on the day of the sports meet, and tug-of-war matches will begin at 12:00 noon.",
  "An individual championship trophy will be awarded in the individual sports categories for participants between ages 11 to 60.",
  "The decisions of the judges and organizers will be final.",
  "No arguments will be entertained. If any disputes arise, that team will be disqualified.",
];

//rules in Kannada
export const rulesKannada = [
  "ಎಲ್ಲಾ ಸ್ಪರ್ಧೆಗಳಲ್ಲಿ 16 ದೇವಸ್ಥಾನಗಳ (ಪದ್ಮಶಾಲಿ) ಸದಸ್ಯರು ಮಾತ್ರ ಭಾಗವಹಿಸಲು ಅವಕಾಶ",
  "ಒಂದು ದೇವಸ್ಥಾನದಿಂದ ವೈಯಕ್ತಿಕ ವಿಭಾಗದಲ್ಲಿ ಪ್ರತಿ ಸ್ಪರ್ಧೆಗೆ ಗರಿಷ್ಠ 3 ಸ್ಪರ್ಧಿ ಮತ್ತು ಒಬ್ಬ ಸ್ಪರ್ಧಿ ಯಾವುದಾದರೂ ಗರಿಷ್ಠ 3 ಸ್ಪರ್ಧೆಗಳಲ್ಲಿ ಭಾಗವಹಿಸಬಹುದು. (ಈ ನಿಯಮ 10 ವರ್ಷ ಒಳಗಿನವರಿಗೆ ಮತ್ತು 60 ವರ್ಷ ಮೇಲ್ಪಟ್ಟವರಿಗೆ ಅನ್ವಯಿಸುವುದಿಲ್ಲ) ಒಂದು ಸ್ಪರ್ಧೆಯಲ್ಲಿ ಕನಿಷ್ಠ 5 ಸ್ಪರ್ಧಿಗಳಿಲ್ಲದಿದ್ದರೆ ಸ್ಪರ್ಧೆಯನ್ನು ಕೈಬಿಡಲಾಗುವುದು.",
  "ಮಹಾಸಭಾದ ನಿಯಮದಂತೆ ದೇವಸ್ಥಾನದವರು ರೂ.1000/-ನ್ನು ನೀಡಿ ನೊಂದಾವಣೆ ಮಾಡಬೇಕು",
  "ಕ್ರೀಡಾಕೂಟವು ಬೆಳಗ್ಗೆ 9-15ಕ್ಕೆ ಎಲ್ಲಾ ವಿಭಾಗಗಳ 800 ಮೀ. ಓಟ ಮತ್ತು ವಾಲಿಬಾಲ್, ಥ್ರೋಬಾಲ್ ಪಂದ್ಯಾಟದೊಂದಿಗೆ ಪ್ರಾರಂಭವಾಗುವುದು.",
  "ವಾಲಿಬಾಲ್, ಥ್ರೋಬಾಲ್ ಮತ್ತು ಹಗ್ಗಜಗ್ಗಾಟ ತಂಡದವರು ಆಧಾರ್ ಕಾರ್ಡ್ ಪ್ರತಿ ಮತ್ತು ದೇವಸ್ಥಾನದ ಪತ್ರದೊಂದಿಗೆ ದಿನಾಂಕ 1-12-2025ರ ಒಳಗೆ ಹೆಸರನ್ನು ಕ್ರೀಡಾ ಸಂಚಾಲಕರು ಅಥವಾ ಕ್ರೀಡಾ ಕಾರ್ಯದರ್ಶಿಯವರಲ್ಲಿ ನೋಂದಾಯಿಸಬೇಕು.",
  "ವಾಲಿಬಾಲ್ ಥ್ರೋಬಾಲ್ ಪಂದ್ಯಗಳು ಕ್ರೀಡೋತ್ಸವ ದಿನದಂದು ಬೆಳಗ್ಗೆ 9-15ರಿಂದ ಮತ್ತು ಹಗ್ಗಜಗ್ಗಾಟ ಪಂದ್ಯಗಳು ಮಧ್ಯಾಹ್ನ 12-00ರಿಂದ ಆರಂಭಗೊಳ್ಳುತ್ತದೆ. 11 ವರ್ಷದಿಂದ 60 ವರ್ಷದವರೆಗಿನ ಕ್ರೀಡೆಗಳಲ್ಲಿ ವೈಯಕ್ತಿಕ ಚಾಂಪಿಯನ್‌ಶಿಪ್ ಪ್ರಶಸ್ತಿ ನೀಡಲಾಗುತ್ತದೆ.",
  "ತೀರ್ಪುಗಾರರ ಮತ್ತು ಸಂಘಟಕರ ತೀರ್ಮಾನವೇ ಅಂತಿಮ. ಯಾವುದೇ ಚರ್ಚೆಗೆ ಅವಕಾಶವಿಲ್ಲ, ಚರ್ಚೆಗಳು ಕಂಡುಬಂದಲ್ಲಿ ಆ ತಂಡವನ್ನು ಕೈ ಬಿಡಲಾಗುವುದು.",
];

//sponsors in English

export const sponsors = [
  {
    name: "Shri Veerabhadra Mahamayi Temple Mulki",
    location: " Mulki",
  },
  {
    name: "D.K. Jilla Padmashali Mahasabha® Mangaluru",
    location: "",
  },
  {
    name: "Shri Veerabhadra Mahamayi Temple Mulki",
    location: " Mulki",
  },
  {
    name: "D.K. Jilla Padmashali Mahasabha® Mangaluru",
    location: "",
  },
];

//sponsors in Kannada

export const sponsorsKannada = [
  {
    name: "ದ.ಕ. ಜಿಲ್ಲಾ ಪದ್ಮಶಾಲಿ ಮಹಾಸಭಾ® ಮಂಗಳೂರು",
    location: "",
  },
  {
    name: "ಶ್ರೀ ವೀರಭದ್ರ ಮಹಮ್ಮಾಯಿ ದೇವಸ್ಥಾನ. ಮಾನಂಪಾಡಿ, ಮುಲ್ಕಿ",
    location: "ಮುಲ್ಕಿ",
  },
  {
    name: "ದ.ಕ. ಜಿಲ್ಲಾ ಪದ್ಮಶಾಲಿ ಮಹಾಸಭಾ® ಮಂಗಳೂರು",
    location: "",
  },
  {
    name: "ಶ್ರೀ ವೀರಭದ್ರ ಮಹಮ್ಮಾಯಿ ದೇವಸ್ಥಾನ. ಮಾನಂಪಾಡಿ, ಮುಲ್ಕಿ",
    location: "ಮುಲ್ಕಿ",
  },
];


//contacts 

//mahasabha contacts in English
export const mahasabhaContacts = [
  { name: "Ravi Shettigar Karkala", phone: "9845249531" },
  { name: "Sadashiva Golijora", phone: "9449269272" },
];

//mahasabha contacts in Kannada
export const mahasabhaContactskannada = [
    { name: "ರವಿ ಶೆಟ್ಟಿಗಾರ್ ಕಾರ್ಕಳ ", phone: "9845249531" },
    { name: "ಸದಾಶಿವ ಗೋಳಿಜಾರ", phone: "9449269272" },
];

//temple contacts in English
export const templeContacts = [
    { name: "Purandar Shettigar", phone: " 9845525125" },
    { name: "Rajesh Sanoor", phone: "7301167711" }, 
];

//temple contacts in Kannada
export const templeContactsKannada = [
    { name: "ಪುರಂದರ ಶೆಟ್ಟಿಗಾರ್", phone: " 9845525125" },
    { name: "ರಾಜೇಶ್ ಸಾಣೂರು", phone: "7301167711" },
];

//event highlights in English
export const eventHighlights = [
  {
    count: "16",
    title: "Temples",
    image: temple,
    ifPlus: "",
  },
  {
    count: "70",
    title: "Events",
    image: torch,
    ifPlus: "+",
  },
  {
    count: "1000",
    title: "Participants",
    image: participants,
    ifPlus: "+",
  },
  {
    count: "200",
    title: "Winners",
    image: winner,
    ifPlus: "+",
  },
];

//event highlights in Kannada
export const eventHighlightsKannada = [
    {
      count: "16",
      title: "ದೇವಸ್ಥಾನಗಳು",
      image: temple,
      ifPlus: "",
    },
    {
      count: "70",
      title: "ಸ್ಪರ್ಧೆಗಳು",
      image: torch,
      ifPlus: "+",
    },
    {
      count: "1000",
      title: "ಸ್ಪರ್ಧಿಗಳು",
      image: participants,
      ifPlus: "+",
    },
    {
      count: "200",
      title: "ವಿಜೇತರು",
      image: winner,
      ifPlus: "+",
    },
  ];

export const events = [
  { year: 1896, place: "Athens" },
  { year: 1900, place: "Paris" },
  { year: 1904, place: "St. Louis" },
  { year: 1908, place: "London" },
  { year: 1912, place: "Stockholm" },
  { year: 1920, place: "Antwerp" },
  { year: 1924, place: "Paris" },
  { year: 1928, place: "Amsterdam" },
];